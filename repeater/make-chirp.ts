...
async function doIt(file: string) {
  const fileData = await readFileAsync(`repeaters/${file}.json`); // await getAllFilesFromDirectory("./repeaters/data/CO/", ".json") as IRepeater[];
...
//   }
//
//   mapped.forEach((m: IChirp, i: number) => m.Location = i + offset);
//
//   const options = {
//     header: true,
//   };
