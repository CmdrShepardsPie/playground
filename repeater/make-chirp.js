...
(function (factory) {
    ...
    // const CC = /CC(\d+)/;
    // const Tone = /(\d+\.?\d*)/;
    function read(files, name, limit = 0, offset = 0, reSort, filter) {
        return __awaiter(this, void 0, void 0, function* () {
            console.log('read', files, name, limit);
            const fileContents = yield Promise.all(files.map(f => fs.readFile(`repeaters/json/${f}.json`)));
            ...
            if (!(yield fs.exists(`chirp/csv/${count}/`))) {
                yield fs.mkdir(`chirp/csv/${count}/`);
            }
            if (!(yield fs.exists(`chirp/json/${count}/`))) {
                yield fs.mkdir(`chirp/json/${count}/`);
            }
            if (!(yield fs.exists(`chirp/csv/nets/`))) {
                yield fs.mkdir(`chirp/csv/nets/`);
            }
            if (!(yield fs.exists(`chirp/json/nets/`))) {
                yield fs.mkdir(`chirp/json/nets/`);
            }
            const netsCsv = yield stringifyAsync(nets, options);
            yield Promise.all([
                fs.writeFile(`chirp/csv/nets/${name}.csv`, netsCsv),
                fs.writeFile(`chirp/json/nets/${name}.json`, JSON.stringify(nets))
            ]);
            const csv = yield stringifyAsync(mapped, options);
            return Promise.all([
    ...
    function makeRow(item) {
        ...
        const row = Object.assign({}, repeater, { 
            // Location,
            Name,
            Frequency,
            Duplex,
